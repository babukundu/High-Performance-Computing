...
int main() {
    ...
    #pragma omp parallel num_threads(NUM_THREADS) // Start parallel region
    {
        ...
        double chunk_start_time = omp_get_wtime();
        ...
        calculateBarycenter(school + startIdx, endIdx - startIdx, &barycenterX, &barycenterY);

        // Update fish weights based on the change in the objective function
        double maxDelta = calculateObjectiveFunction(school + startIdx, endIdx - startIdx);
        #pragma omp critical
        updateFishWeights(school + startIdx, endIdx - startIdx, maxDelta);

        // Record the end time for this chunk and print the execution time for this chunk
        double chunk_end_time = omp_get_wtime();
        printf("Execution Time for Chunk %d: %.6lf seconds\n", threadID, chunk_end_time - chunk_start_time);
